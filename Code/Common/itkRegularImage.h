...
=========================================================================*/
#ifndef __itkRegularImage_h
#define __itkRegularImage_h

#include "itkRegularImageBase.h"
#include "itkRegularImageRegion.h"
#include "itkImportRegularImageContainer.h"
 ...
 *
 * RegularImages are templated over a pixel type (modeling the dependent
 * variables), and a dimension (number of independent variables).  The
 * container for the pixel data is the ImportRegularImageContainer.
 *
 ...
 *                           C [RegularImageIndex, RegularImageSize]
 *
 ...
 * ...
 *
 * \sa RegularImageContainerInterface
 *
 * \example DataRepresentation/RegularImage/RegularImage1.cxx
 * \example DataRepresentation/RegularImage/RegularImage2.cxx
 * \example DataRepresentation/RegularImage/RegularImage2.cxx
 * \example DataRepresentation/RegularImage/RGBRegularImage.cxx
 * \example DataRepresentation/RegularImage/VectorRegularImage.cxx
 *
 * \ingroup RegularImageObjects */
template< class TPixel, unsigned int VRegularImageDimension = 2 >
class ITK_EXPORT RegularImage:public RegularImageBase< VRegularImageDimension >
{
public:
  /** Standard class typedefs */
  typedef RegularImage                               Self;
  typedef RegularImageBase< VRegularImageDimension > Superclass;
  ...
  itkTypeMacro(RegularImage, RegularImageBase);
   ...
   * with RegularImage Adaptors and allow to keep a particular internal
   ...
   * the image. */
  itkStaticConstMacro(RegularImageDimension, unsigned int, VRegularImageDimension);

  /** Container used to store pixels in the image. */
  typedef ImportRegularImageContainer< unsigned long, PixelType > PixelContainer;
   ...
   * RegularImageSource::GraftOutput(). The implementation in RegularImageBase
...
private:
  RegularImage(const Self &);          //purposely not implemented
...
#define ITK_TEMPLATE_RegularImage(_, EXPORT, TypeX, TypeY)     \
  namespace itk                                         \
  {                                                     \
  _( 2 ( class EXPORT RegularImage< ITK_TEMPLATE_2 TypeX > ) ) \
  namespace Templates                                   \
  {                                                     \
  typedef RegularImage< ITK_TEMPLATE_2 TypeX > RegularImage##TypeY; \
  }                                                     \
  }

#if ITK_TEMPLATE_EXPLICIT
//template <class TPixel, unsigned int VRegularImageDimension> const unsigned int
// itk::RegularImage<TPixel,VRegularImageDimension>::RegularImageDimension;
#include "Templates/itkRegularImage+-.h"
#endif

#if ITK_TEMPLATE_TXX
#include "itkRegularImage.txx"
#endif
