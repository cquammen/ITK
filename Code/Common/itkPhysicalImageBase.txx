...
::UpdateOutputData()
{
  ...
  // PhysicalImageBase as  oppose to DataObject, but cause this statement
...
{
  typedef PhysicalImageBase< VImageDimension > PhysicalImageBaseType;

  const PhysicalImageBaseType *image;

  try
    {
    image = dynamic_cast< const PhysicalImageBaseType * >( data );
    }
  catch ( ... )
    {
    return;
...
{
  PhysicalImageBase *imgData;

  imgData = dynamic_cast< PhysicalImageBase * >( data );
